...
/*
    ...
    * op就是操作的类型 ADD，：需要监听什么就把它加进去
                      DEL：不需要这个fd就把体删除
                      MOD：把它这个fd给修改掉,把EPOLLIN，改成EPOLLOUT
...
1. 凡是在红黑树中的节点，对应的fd和事件，就是OS系统需要关心的
2. 进程是怎么知道系统有数据的，通过中断的方式来提示cpu
            ...
            if (events[i].events & EPOLLIN)
            {
                ...
                else //不是监听文件描述符，普通文件描述符就有IO操作
                {
                    ...
                    if (size <= 0)
                    {
                        ...
                        ev.events = EPOLLIN;
                        epoll_ctl(epolfd, EPOLL_CTL_DEL, ev.data.fd, nullptr); //删除这个文件描述符,就不关心它的事件了
                        close(events[i].data.fd);                          //关闭
                        cout<<"delete sockfd success"<<endl;
                    ...
                    else
                    {
                        buf[size] = 0;
                        printf("hello %s\n", buf);
                        //这个之后就可以把EPOLLIN改成EPOLLOUT，这里已近读完了
                        struct epoll_event ev;
                        ev.events=EPOLLOUT;//把关心的事件修改成epollout
                        ev.data.fd=events[i].data.fd;
                        epoll_ctl(epolfd,EPOLL_CTL_MOD,events[i].data.fd,&ev);
                        //这样设置完之后，就只关心这个文件描述符上面的写事件
